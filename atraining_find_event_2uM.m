...
% close all
% clear all
file_start = "Pore2_5000bps_1MKCl_-200mV_20201027_174200_EXPORT";
%file_start = "Pore2_5000bps_1MKCl_-200mV_";
...
for file = 1:length(Files)
    ...
    addFile_med = smoothdata(addFile(:, 2), 'movmed', 20000);
    addSignal = [(running_index+1:running_index+length(addFile_med)).', addFile(:, 2)-addFile_med];
    running_index = running_index + length(addFile_med);
    TwoMHz_wrap = [TwoMHz_wrap; addSignal];
    disp(file+"/"+length(Files)+" Loaded length " + length(addFile_med) + ", " + Files(file).name);
end
...
figure(1)
plot(TwoMHz(:,2))
title("Note maximum noise level and record");
...
k=1;
m=0;
baseline_threshold = -4.0e-10; %MUST BE CHANGED TO CHANGE NOISE LEVEL
%cat1_data = cell(7, 0);
%cat2_data = cell(7, 0);
%cat3_data = cell(7, 0);
%cat4_data = cell(7, 0);
while j < length(n)
    ...
    if req1% && req2
        ...
        mydata{1,k} = TwoMHz(event_left:event_right,1); %x-values first
        mydata{2,k} = TwoMHz(event_left:event_right,2); %y-values second
        mydata{3,k} = TwoMHz(event_left-40:event_right+40, 1);
        mydata{4,k} = TwoMHz(event_left-40:event_right+40, 2); %surrounding y-values
        mydata{5,k} = k;
